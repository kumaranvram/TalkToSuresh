...
static NSString * const kTalkToSureshJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_talktosureshInjection
{
	...
	[html appendString:kTalkToSureshJavascript];
...
{
	// attempt to make well-formed HTML and inject in our TalkToSuresh bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _talktosureshInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to TalkToSuresh support.",url);
				}
			}
